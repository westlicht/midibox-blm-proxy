...
public:  
  ...
  public:
    ...
    bool isMidi() { return currentTypeTag() == TYPE_TAG_MIDI; }
    ...
    /** retrieve a midi argument */
    ArgReader &popMidi(uint32_t &i) { return popPod<uint32_t>(TYPE_TAG_MIDI, i); }
  ...
  */
  ...
  Message &pushMidi(uint32_t i) { return pushPod(TYPE_TAG_MIDI, i); }
...
private:
  ...
  friend std::ostream &operator<<(std::ostream &os, const Message &msg) {
    ...
    while (arg.nbArgRemaining() && arg.isOk()) {
      ...
      else if (arg.isMidi()) { uint32_t i; arg.popMidi(i); os << i; }
      else {
